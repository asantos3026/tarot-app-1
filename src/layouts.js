const CELTIC_CROSS = [
  { x: 2, y: 3, sideways: false },
  { x: 2, y: 3, sideways: true },
  { x: 2, y: 5, sideways: false },
  { x: 0, y: 3, sideways: false },
  { x: 2, y: 1, sideways: false },
  { x: 4, y: 3, sideways: false },
  { x: 6, y: 6, sideways: false },
  { x: 6, y: 4, sideways: false },
  { x: 6, y: 2, sideways: false },
  { x: 6, y: 0, sideways: false }
]

const CELTIC_PYRAMID = [
  { x: 3, y: 0, sideways: false },
  { x: 2, y: 2, sideways: false },
  { x: 4, y: 2, sideways: false },
  { x: 1, y: 4, sideways: false },
  { x: 3, y: 4, sideways: false },
  { x: 5, y: 4, sideways: false },
  { x: 0, y: 6, sideways: false },
  { x: 2, y: 6, sideways: false },
  { x: 4, y: 6, sideways: false },
  { x: 6, y: 6, sideways: false }
]

export { CELTIC_CROSS, CELTIC_PYRAMID }